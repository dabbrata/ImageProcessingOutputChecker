...
pts = getPoints(magnitude_shift,4)

ideal_filter = np.zeros((img.shape[0],img.shape[1]),dtype = np.float32)
...
for i in range(img.shape[0]):
    for j in range(img.shape[1]):
        ...
        for k in range(len(v)):
            ...
            if(duv < d0 or dmuv<d0):
                prod=0
                break
        ideal_filter[i,j] = prod
        
        
plt.figure()
plt.imshow(ideal_filter,'gray')
plt.title("ideal notch filter")

G_S = F_shift*ideal_filter
G = np.fft.ifftshift(G_S)
output = np.fft.ifft2(G).real

plt.figure()
plt.imshow(output,'gray')
plt.title('ideal notch filtered image')
plt.show()